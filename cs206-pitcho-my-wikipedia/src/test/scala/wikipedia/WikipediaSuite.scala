...
class WikipediaSuite {
  ...
  /**
    ...
    */
  def assertSameElements[A](givn: List[A], expected: List[A]): Unit = {
    val givenSet = givn.toSet
    ...
    val noMatchString =
      s"""
         ...
         |Actual:   ${previewList(givn)}""".stripMargin
  ...
  def assertEquivalentAndOrdered(givn: List[(String, Int)], expected: List[(String, Int)]): Unit = {
    // (1)
    assertSameElements(givn, expected)
    // (2)
    assert(
      !(givn zip givn.tail).exists({ case ((_, occ1), (_, occ2)) => occ1 < occ2 }),
