...
import FloatOps._
  ...
  @Test def `Fork with 3 empty quadrants and 1 leaf (nw & ne)`: Unit = {
    {
      val b = new Body(123f, 18f, 26f, 0f, 0f)
    ...
    {
      val b = new Body(123f, 20.5f, 26f, 0f, 0f)
      ...
      assert(quad.centerY == 30f, s"${quad.centerY} should be 30f")
      assert(quad.mass ~= 123f, s"${quad.mass} should be 123f")
      assert(quad.massX ~= 20.5f, s"${quad.massX} should be 18f")
      assert(quad.massY ~= 26f, s"${quad.massY} should be 26f")
      assert(quad.total == 1, s"${quad.total} should be 1")
    }
  }

  @Test def `Empty.insert(b) should return a Leaf with only that body`: Unit = {
    val quad = Empty(51f, 46.3f, 5f)
  ...
  @Test def `Body.updated should take bodies in a Leaf into account`: Unit = {
    val b1 = new Body(123f, 18f, 26f, 0f, 0f)
  ...
  @Test def `'SectorMatrix.+=' should add a body at (25,47) to the correct bucket of a sector matrix of size 96`: Unit = {
